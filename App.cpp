...
void Main::file_load_chunk(const std::string &name, std::streampos start, std::streamsize size) {
    std::ifstream file;
...
void Main::on_file_load_finish(const Glib::RefPtr<Gio::AsyncResult>& result, const Glib::RefPtr<Gtk::FileDialog>& dialog)
{
    try
    {
...
void Main::file_save_chunk(const std::string &filename, std::streampos start, std::streamsize size) {
...
void Main::on_file_save_finish(const Glib::RefPtr<Gio::AsyncResult>& result, const Glib::RefPtr<Gtk::FileDialog>& dialog)
{
...
void Main::xor_chunk(std::deque<char> &file_data, const std::string &key, size_t start, size_t end) {
    for (size_t i = start; i < end; i ++) {
        file_data[i] ^= key[i % key.size()];
    }
}
void Main::xor_crypt(const std::string &key)
{
    size_t num_threads = std::thread::hardware_concurrency();
...
void Main::xor_decrypt(const std::string &key)
{
    file_data.pop_back();
    file_data.pop_back();
